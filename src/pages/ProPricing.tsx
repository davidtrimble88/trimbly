    ...
    if (tierKey === "pro" && hasProvider) {
      ...
      if (pendingFlexCode) {
        ...
        if (result?.success) {
          ...
          toast({ title: "Pro activated!", description: "Applied your discount code — no payment needed." });
  ...
  return (
    <div className="min-h-screen">
      ...
      <VikingModeUnlockedDialog
        ...
        onContinue={() => { setShowVikingUnlock(false); navigate(hasProvider ? "/pro-dashboard" : "/pro-register?tier=pro"); }}
      />
    </div>
  );
};
