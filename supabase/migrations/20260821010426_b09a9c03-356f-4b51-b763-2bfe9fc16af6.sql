...
-- whenever street_address is blank (not just when all three fields are
-- blank) so existing rows and any entry point that hasn't collected a
-- street address yet never collide.
ALTER TABLE public.homes
  ADD COLUMN IF NOT EXISTS street_address_normalized text
  GENERATED ALWAYS AS (
    CASE
      WHEN coalesce(street_address, '') = '' THEN NULL
      ELSE regexp_replace(lower(coalesce(street_address, '') || '|' || coalesce(city, '') || '|' || coalesce(state, '')), '[^a-z0-9|]', '', 'g')
    END
  ) STORED;
