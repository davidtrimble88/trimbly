...
-- Returns a small jsonb result the frontend can branch on rather than a
-- bare error, since "invalid", "expired", "already used" etc. all need
...
-- Lets a user set their OWN subscription_tier to one of the three real
-- tiers — this is what HomeownerUpsell.tsx's tier-selection buttons need
-- (the early-access "Start 14-Day Free Trial" flow, which has no real
-- Stripe checkout yet and was never meant to be payment-gated during this
-- period). Was previously done as a raw client-side .update(), which
-- protect_profile_privileged_columns() silently reverted for every
-- non-admin/moderator user — the buttons appeared to work (toast fired) but
-- never actually changed anything. Scoped narrowly (own row only, fixed
-- allow-list of tiers) so it can't be used to touch anyone else's account
-- or set any other privileged column.
